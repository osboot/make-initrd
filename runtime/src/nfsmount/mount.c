...
static int get_ports(uint32_t server, const struct nfs_mount_data *data)
{
	...
	if (nfs_port == 0) {
		port = portmap(server, NFS_PROGRAM, nfs_ver, proto);
		if (port == 0 || port > INT_MAX) {
			if (port > INT_MAX) {
				fprintf(stderr, "NFS Port number is too large");
				return -1;
			}
	...
	if (mount_port == 0) {
		port = portmap(server, NFS_MNT_PROGRAM, mount_ver, proto);
		if (mount_port == 0 || port > USHRT_MAX)
			mount_port = MOUNT_PORT;
		mount_port = (uint16_t) port;
	}
	return 0;
}

static inline size_t pad_len(size_t len)
{
	return (len + 3) & ~3;
}
...
static inline void dump_fh(const char *data, size_t len)
{
	(void)data;
	(void)len;
