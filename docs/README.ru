...
Большинство скриптов сборки образа используют libshell - набор часто используемых
в shell функций.

kinit-utils - Набор утилит из klibc, спортированные на glibc. Эти утилиты имеют
широкий функционал и маленькие размеры (даже собранные с glibc).
...
FEATURES      - Этот параметр перечисляет список особенностей образа. Этот
                ...
                сжать образ определённым способом, а затем необходимо удалить
                все временные файлы от создания образа.

AUTODETECT    - В этой переменной перечисляются подсистемы, для которых
                автоматичеки будут находиться модули ядра и добавляться фичи.
                Если эта переменная будет пустой, то автодетеск будет отключён.
                ...
                resume - будут добавлены модули для устройства с которого
                         ...
                         устройство зашифровано или находится в неожиданном месте.

                common - добавляет фичи общего назначения. Если вы не знаете, что
                        делаете, то не отключайте эту особенность.
                ...
                net   - будут добавлены модули ядра для загрузки сетевых устройств
                        ...
                        net:iface=<iface> - указывает имя интерфейса, для которого
                ...
                device - будут добавлены модули необходимые для использованияa
                         устройства. В качестве параметра необходимо указать путь
                         к устройству на файловой системе sysfs (без /sys).
                         Например:
...
Особенности - это части функционала, которые выполняется при создании образа или
...
devmapper     - Добавить поддержку device-mapper'а. Это особенносить также добавляется
                при искользовании lusk или lvm.
...
lazy-cleanup  - Удалять временные файлы, не после, а до создания образа. Бывает полезно
                при отладке.
...
guess-config  - При вызыве этой цели на экран (stdout) предполагаемый конфиг.
                ...
                Эта цель удобна для создания первогоначального конфига в новой
                системе.
...
bug-report    - Цель собирает данные о конфигурации системы и запаковывет
                ...
                то эта цель поможет собрать информацию, необходимую разработчикам,
...
на несколько стадий. На самом деле, количество статий не регламентировано.
