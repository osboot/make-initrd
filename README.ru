...
особенностей (фич) будущего образа. При этом порядок указания совершенно не
важен. Пользователь не обязан знать как и в каком порядке будут применяться
...
AUTODETECT    - В этой переменной перечисляются подсистемы, для которых
                ...
                common - добавляет фичи общего назначения. Если вы не знаете,
                         что делаете, то не отключайте эту особенность.
...
devmapper     - Добавить поддержку device-mapper'а. Это особенность также
                добавляется при искользовании lusk или lvm.
